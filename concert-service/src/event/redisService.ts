  ...
  private async initializeResponseListener(): Promise<void> {
    ...
    const processResponseRead = async () => {
      try {
        ...
        const messages = await redisStreamOperations.readFromStream(requestStreamRead, '0-0');
        for (const msg of  messages) {
          try {
            const { userId, concertId, seatType } = msg.data as { userId: string, concertId: string, seatType: string };
            const consert = await concertService.findById(concertId);
            const message = {
              concert: consert,
              requestId: msg.id,
            }
            await redisStreamOperations.publishToStream(responseStreamRead, message);
          } catch (error) {
