...
export class ConcertService {
  ...
  async create(data: ConcertInput | ConcertInput[]): Promise<Concert | Concert[]> {
  ...
  async update(id: string, data: Partial<ConcertInput>): Promise<Concert | null> {
    if (!Types.ObjectId.isValid(id)) return null;
    return ConcertModel.findByIdAndUpdate(id, data, { new: true }).lean().exec();
  }
